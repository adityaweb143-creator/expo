    ...
    return {
        ...
        visitor: {
            ...
            ExportNamedDeclaration(path, state) {
                // NOTE(@hassankhan): Server bundles keep loaders for SSG's executeLoaderAsync.
                // A followup is required to strip loaders from SSR server bundles.
                ...
                debug(`Processing ${isLoaderBundle ? 'loader-only' : 'client'} bundle:`, state.file.opts.filename);
                ...
                if (t.isFunctionDeclaration(declaration)) {
                    ...
                    if (name && isLoaderIdentifier(name)) {
                        ...
                        if (!isLoaderBundle) {
                            // Client and SSR bundles: remove loader
                ...
                if (t.isVariableDeclaration(declaration)) {
                    ...
                    if (isLoaderBundle) {
                        ...
                        // Client and SSR bundles: remove loader declarations
                        declaration.declarations = declaration.declarations.filter((declarator) => {
                            const name = t.isIdentifier(declarator.id) ? declarator.id.name : null;
                            if (name && isLoaderIdentifier(name)) {
